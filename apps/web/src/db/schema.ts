...
import { pgTable, varchar, text, smallint, integer, primaryKey } from 'drizzle-orm/pg-core';
...
export const rankAverage = pgTable('ranksAverage', {
  personId: varchar('personId', { length: 10 }).notNull().references(() => person.id),
  eventId: varchar('eventId', { length: 6 }).notNull().references(() => event.id),
...
export const rankSingle = pgTable('ranksSingle', {
  personId: varchar('personId', { length: 10 }).notNull().references(() => person.id),
  eventId: varchar('eventId', { length: 6 }).notNull().references(() => event.id),
  best: integer('best').notNull().default(0),
  worldRank: integer('worldRank').notNull().default(0),
...
export const result = pgTable('results', {
  competitionId: varchar('competitionId', { length: 32 }).notNull().references(() => competition.id),
  eventId: varchar('eventId', { length: 6 }).notNull().references(() => event.id),
  ...
  return [{
    pk: primaryKey({ columns: [table.competitionId, table.eventId, table.roundTypeId, table.pos] }),
  }];
});
